...
#### 问题1：
编译器是所有的代码段都编译完才引擎运行，还是一句一句的交替执行

#### 问题2：
有没有工具像gcc一样看到javascript编译后的输出

>
    js的编译原理要深入理解，周爱民一本JavaScript语言精髓与编程实践要抽空看下
    网上的文章http://blog.csdn.net/yhawaii/article/details/7221568 也不错
...
```
#### 问题3 
这里b在什么阶段被认为挂在了window 上面从下文看应该是引擎运行阶段，当作为lhs时所有的作用域都找不到，就挂在了全局作用域中，所以它之上的a+b 就找不到了
>
经过测试即使在全局增加 var b; 最后结果竟然是NAN，我理解应该是2undefined的字符串，因为+的缘故
